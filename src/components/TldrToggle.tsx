  ...
  return (
    <div className="flex items-center">
      <button
        ...
        className={`px-3 py-1.5 rounded-lg border text-xs font-mono font-medium transition-all duration-300 flex items-center gap-2 shadow-xs cursor-pointer ${
          isActive
            ? 'bg-accent text-accent-text border-accent ring-2 ring-accent/30'
      ...
      >
        <span
          className={`w-2 h-2 rounded-full transition-colors ${
            isActive ? 'bg-emerald-400 animate-pulse' : 'bg-accent'
          }`}
        />
        <span>TLDR</span>
        <span
